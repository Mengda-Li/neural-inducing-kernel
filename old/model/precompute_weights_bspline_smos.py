...
def dblquad_montecarlo_jacobian(u, v, xis_ctr, yas_ctr, dxi, dya, n=100, R=R, h=h):
	...
	xis_sample = xis_rel + np.tile(xis_ctr.reshape((ngrid,1)), (1,n)) #np.hstack([np.zeros((ngrid,1)), xis_rel]) + np.tile(xis_ctr.reshape((ngrid,1)), (1,n))
	yas_sample = yas_rel + np.tile(yas_ctr.reshape((ngrid,1)), (1,n)) #np.hstack([np.zeros((ngrid,1)), yas_rel]) + np.tile(yas_ctr.reshape((ngrid,1)), (1,n))
...
# major params
for decimation_factor_y in [8]: # can be fractional due to upsampling zoom
	N = 21600/decimation_factor_y
	try:
		for number_components in [3]:
			cexp_weights = {}
			for zoom in [1]:
				for dxi, dya in zip([0.02], [2*np.pi/(N*zoom)]):
					cnt = 0
					offset_xi = 0
					...
					for b in baselines:
						cexp_weights[(np.around(dxi,10), np.around(dya,10), np.around(b[0],10), np.around(b[1],10))] = dblquad_montecarlo_jacobian(b[0], b[1], xi_vis, yang_vis, dxi, dya, n=n, R=R, h=h)
						cnt += 1
						if cnt % 100 == 1:
							print(f"{cnt/len(baselines):.4f} fraction done")
					with open(f"smos_weights_{dxi:.6f}_{dya:.6f}_n_{n}.npy", "wb") as f:

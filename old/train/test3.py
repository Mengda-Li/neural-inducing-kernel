...
print("NVIS", nvis, "NANT", nant)

# get xi-eta sampling grid

visibilities = {}

Nts =  np.arange(128, 1404, 4)
Nts_special = [128, 196, 1024, 1400]
for Nt in Nts:
    print(Nt)
    ...
    visibilities[Nt] = (hkl @ img_vals)*(1/Nt)*(12/(Nt*np.sqrt(3)))

plt.figure()
for Nt in Nts:
    if Nt in Nts_special:
        plt.scatter(np.real(visibilities[Nt][1]), np.imag(visibilities[Nt][1]), label=f'{Nt}x{Nt}', marker='x', s=4, alpha=0.7)
    else:
        plt.scatter(np.real(visibilities[Nt][1]), np.imag(visibilities[Nt][1]), c='gray', alpha=0.2, marker='.', s=2)
plt.xlabel(r"real part, $19.25\sqrt{{3}}$ visibility")
plt.ylabel(r"imaginary part, $19.25\sqrt{{3}}$ visibility")
plt.title(f"Evolution of 19.25$\sqrt{{3}}$ Visibility with Hexagonal Grid Size")
plt.annotate(r'$V_{k,l}=$'+f'{np.real(visibilities[Nt][1]):.3f}'+r' + '+f'{np.imag(visibilities[Nt][1]):.3f}'+r'i', xy=(0.05,0.05), xycoords='axes fraction')
plt.legend()
plt.savefig(f"visibilities_err_plt_evolution_hexagon.png")
plt.close()

with open('xietavisibilities.pkl', 'wb') as outf:
    pickle.dump(visibilities, outf)

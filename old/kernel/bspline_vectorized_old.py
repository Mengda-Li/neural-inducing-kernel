...
def get_vis_one_pixel_stride_vectorized(param_img,deg,knots,Ti,Tf,
	...
	vis_with_mean = np.zeros((Nvis, Tf-Ti+1), dtype=np.complex128)
...
for Nx in [33]:
	for Ny in [33,17,9]:
		...
		# major params
		for decimation_factor_x, decimation_factor_y in zip([30,10,3,1],[30,10,3,1]):
			try:
				...
				for number_components in [3]:
					...
					if len(img2.shape) == 2:
						...
						for c in range(img2.shape[2]):
							...
							plt.close()
					for dxi in [0.02,0.01,0.005]: #[0.02, 0.01, 0.005]:
						offset_xi = 0
						...
						print("ngrid", ngrid, "nvis", nvis, "xi_vis", xi_vis.shape)
						...
						# antenna product
						antpattern_matrix = assign_and_resample_antenna_pattern_matrix(ps, np.pi-ts, points_spatial, delta_xi=dxi, delta_y=2*np.pi/N, seed=10,thresh=10,ablation_antenna=False, scenario_name=f'{Nx}x{Ny}rectangularframe')
						...
						print("nvis", nvis, "nant", nant, "len(points_spatial)", len(points_spatial))
						...
						# hkl
						print("J", J.shape)
						print("antpattern_matrix", Amat.shape)
						print("cexp", cexp.shape)
						hkl = cexp*Amat*J
						...
						for number_knots in [3,9]:
							for deg in [3]:
								...
								for Ti, Tf in zip([0], [param_img.shape[1]-1]):
									Mh = param_img.shape[0]
									No = param_img.shape[1]
									C = 1 if len(param_img.shape) == 2 else param_img.shape[2]
									add_noise = True
									fname = f"full_noise_{add_noise}_rect_Ti_{Ti}_Tf_{Tf}_Ny_{Ny}_Nx_{Nx}_du_{du}_dv_{dv}_dxi_{dxi}_dfx_{decimation_factor_x}_dfy_{decimation_factor_y}_reg_{regularization}_lambda_{l_}_nknots_{number_knots}_deg_{deg}_numbcomp_{number_components}.pkl"
									vis, vtilde, vis_with_mean = get_vis_one_pixel_stride_vectorized(param_img,deg,knots,Ti,Tf,points_spatial, xi_vis, eta_vis, xang_vis, yang_vis,hkl, Umean, U, var=var,n_components_simulation=number_components, fname="vec_vis_"+fname)
									if add_noise:
										Bwidth = 20e6
										tau = 1
									...
									param_est, I, Gs, param_est_full = invert_vis_one_pixel_stride_vectorized(vtilde,deg,knots,Ti,Tf,Mh,No,C,xang_vis,yang_vis,xi_vis,eta_vis,hkl,U,points_spatial,n_components_inversion=number_components,thresh=10,ablation_cexp=False,fname="vec_inv_"+fname)
									...
									if len(param_est.shape) == 2:
										...
										plt.title("Image of spline weights on BT params")
										plt.colorbar()
										plt.savefig( f"recovered_param_img_component_1_oo_1_"+fname[:-4]+".png")
										plt.close()
										...
										plt.colorbar()
										plt.savefig(f"error_in_param_img_component_1_oo_1"+fname[:-4]+".png")
										plt.close()

										plt.figure()
										plt.imshow(np.log10(abs(param_est-param_img[:,Ti:Tf+1])))
										plt.axis('off')
										plt.title("Log error image of spline weights on BT params")
										plt.colorbar()
										plt.savefig(f"log_error_in_param_img_component_1_oo_1"+fname[:-4]+".png")
										plt.close()



									elif len(param_img.shape) ==3:
										pi = multiply_channels_indep(B, param_img[:,Ti:Tf+1])
										pe = multiply_channels_indep(B, param_est)
										...
										temp27p5_sim = compute_t_theta(pi,4,mean_vec=mean_vec,var=var,pcs=pcs,nparam=number_components)
										temp27p5_rec = compute_t_theta(pe,4,mean_vec=mean_vec,var=var,pcs=pcs,nparam=number_components)
										...
										mm,nn = B.shape[0], param_est.shape[1]
										print("mm,nn", mm, nn, temp57p5_rec.shape, temp57p5_sim.shape)
										synth = np.zeros((mm,nn))
										synth[:,:nn//3] = np.abs(temp2p5_sim-temp2p5_rec)[:,:nn//3]
										synth[:,nn//3:2*nn//3] = np.abs(temp27p5_sim-temp27p5_rec)[:,nn//3:2*nn//3]
										synth[:,2*nn//3:] = np.abs(temp57p5_sim-temp57p5_rec)[:,2*nn//3:]
										fig, ax = plt.subplots(1)
										cax = ax.imshow(np.log10(synth[5:-5]))
										plt.ylabel("Error in BT (K)")
										plt.title(r"Reconstructed BT error ($C=$" + f"{number_components}, " + r"$B\cdot\tau=$" + f"{Bwidth*tau:.1e})")
										plt.xlabel(r"Simulated & Recovered BT curves sampled at [2.5$^\circ$ | 22.5$^\circ$ | 57.5$^\circ$]")
										ax.xaxis.set_ticks([])
										ax.yaxis.set_ticks([])
										ax.xaxis.set_ticklabels([])
										ax.yaxis.set_ticklabels([])
										cbar = fig.colorbar(cax, fraction=0.0195, pad=0.04, ticks=[-8,-7,-6,-5,-4, -3, -2, -1, 0, 1, 2, 3, 4])
										cbar.ax.set_yticklabels([r'$10^{-8}$', r'$10^{-7}$',r'$10^{-6}$',r'$10^{-5}$',r'$10^{-4}$', r'$10^{-3}$', r'$10^{-2}$', r'$10^{-1}$', r'$10^{0}$', r'$10^{1}$', r'$10^{2}$', r'$10^{3}$', r'$10^{4}$'])  # vertically oriented colorbar
										plt.savefig("temp_err_"+fname[:-4]+".png")
										plt.close()
										...
										cax = ax.imshow(np.log10(synth[5:-5]))
										plt.ylabel("Error in BT (K)")
										plt.title(r"Projn & Reconstn BT error ($C=$" + f"{number_components}, " + r"$B\cdot\tau=$" + f"{Bwidth*tau:.1e})")
										plt.xlabel(r"Simulated & Recovered BT curves sampled at [2.5$^\circ$ | 22.5$^\circ$ | 57.5$^\circ$]")
										ax.xaxis.set_ticks([])
										ax.yaxis.set_ticks([])
										ax.xaxis.set_ticklabels([])
										...
										cbar = fig.colorbar(cax, fraction=0.0195, pad=0.04, ticks=[-8,-7,-6,-5,-4, -3, -2, -1, 0, 1, 2, 3, 4])
										cbar.ax.set_yticklabels([r'$10^{-8}$', r'$10^{-7}$',r'$10^{-6}$',r'$10^{-5}$',r'$10^{-4}$', r'$10^{-3}$', r'$10^{-2}$', r'$10^{-1}$', r'$10^{0}$', r'$10^{1}$', r'$10^{2}$', r'$10^{3}$', r'$10^{4}$'])  # vertically oriented colorbar
										plt.savefig("reconstruct_temp_err_"+fname[:-4]+".png")
										...
										for c in range(param_img.shape[2]):
											...
											plt.title("Image of spline weights on BT params")
											plt.colorbar()
											plt.savefig( f"recovered_param_img_"+fname[:-4]+f"component_{c+1}_out_of_{param_img.shape[2]}.png")
											plt.close()

											plt.figure()
											plt.imshow(np.abs(param_est[:,:,c]-param_img[:,Ti:Tf+1,c]))
											plt.axis('off')
											plt.title(f"Abs error of col spline weights, param no {c+1}")
											plt.colorbar()
											plt.savefig(f"error_in_param_img_"+fname[:-4]+f"_component_{c+1}_out_of_{param_img.shape[2]}.png")
											plt.close()

											plt.figure()
											plt.imshow(np.log10(np.abs(param_est[:,:,c]-param_img[:,Ti:Tf+1,c])))
											plt.axis('off')
											plt.title(f"Log error of col spline weights, param no {c+1}")
											plt.colorbar()
											plt.savefig(f"log_error_in_param_img_"+fname[:-4]+f"_component_{c+1}_out_of_{param_img.shape[2]}.png")
											plt.close()
											...
											plt.colorbar()
											plt.savefig(f"resampled_param_img_"+fname[:-4]+f"_component_{c+1}_out_of_{param_img.shape[2]}.png")
											plt.close()

											plt.figure()
											print("B", B.shape, "param_est", param_est.shape, "param_img", param_img.shape, "img horizon", img_horizon.shape)
											plt.imshow(np.abs(B @ param_est[:,:,c])-img_horizon[:,Ti:Tf+1,c])
											plt.axis('off')
											plt.title(f"Error in interpolated BT params, param no {c+1}")
											plt.colorbar()
											plt.savefig(f"error_in_interpolated_img_"+fname[:-4]+f"_component_{c+1}_out_of_{param_img.shape[2]}.png")
											plt.close()

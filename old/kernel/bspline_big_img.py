...
def load_weights(xi_sample, ya_sample, baselines, dxi, dya, n):
	fname = f"smos_weights_{dxi:.6f}_{dya:.6f}_n_{n}.npy"
	with open(fname, 'rb') as f:
		d = pickle.load(f)
	res = np.zeros((len(baselines), len(xi_sample)), dtype=np.complex128)
	print("loading weights, expected shape", res.shape, "actual shape", d[(np.around(dxi,10), np.around(dya,10), np.around(baselines[0,0],10), np.around(baselines[0,1],10))].shape)
	print("d", d.keys())
	for i,b in enumerate([tuple(ab) for ab in baselines]):
		res[i,:] = d[(np.around(dxi,10), np.around(dya,10), np.around(b[0],10), np.around(b[1],10))]
	return res
...
def invert_vis_z(vtilde,deg,knots,Ti,Tf,s,z,Nys,L,N,C,xa_rs,ya_rs,xi_rs,eta_rs,dya,hkl_tiled,
...
save_g = False	
# major params
for decimation_factor_x, decimation_factor_y in zip([1],[1]):
	try:
		print("Got here!", decimation_factor_x, decimation_factor_y)
		for number_components in [3]:
			# import image 
			img = mpimg.imread('ultrabluemarble.png')/255.0
			...
			for dxi, dya in zip([0.01], [2*np.pi/N]):
				offset_xi = 0
				...
				for number_knots, deg in zip([165], [3]):
					for reg_ in [0.001]:
						regularization= True
						l_ = reg_ if regularization else 0
						...
						cexp = load_weights(xi_vis, yang_vis, baselines, dxi, dya, n=50)

						# antenna product
						antpattern_matrix = assign_and_resample_antenna_pattern_matrix(ps, np.pi-ts, points_spatial, delta_xi=dxi, delta_y=dya, 
																						seed=10, thresh=10, ablation_antenna=False, scenario_name=f'SMOS_instrument_rectangular_grid_{dxi}x{N}')
						...
						for Ti, Tf, s in zip([0], [param_img.shape[1]//10-1], [8]):
							add_noise = False
							fname = f"big_img_full_noise_{add_noise}_rect_Ti_{Ti}_Tf_{Tf}_stride_{s}__dxi_{dxi}_dya_{dya:.4f}_dfx_{decimation_factor_x}_dfy_{decimation_factor_y}_reg_{regularization}_lambda_{l_}_nknots_{number_knots}_deg_{deg}_numbcomp_{number_components}.pkl"
							...
							if save_g:
								param_est, I, Gs, param_est_full = invert_vis_z(vtilde,deg,knots,Ti,Tf,s,1,Nys,L,N,C,xang_vis,yang_vis,xi_vis,
																		eta_vis,dya,hkl,Tcs,n_components_inversion=number_components,
																		thresh=10,fname="vec_inv_"+fname, save_g=save_g)
							else:
								param_est, I, param_est_full = invert_vis_z(vtilde,deg,knots,Ti,Tf,s,1,Nys,L,N,C,xang_vis,yang_vis,xi_vis,
								eta_vis,dya,hkl,Tcs,n_components_inversion=number_components,
								thresh=10,fname="vec_inv_"+fname, save_g=save_g)

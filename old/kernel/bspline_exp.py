...
def get_img_of_params_bspline(img,xang_vis,number_knots,deg,d=np.arccos(R/(R+h)), margin=0.02, regularization=False, l=0):
    ...
    if regularization:
        param_img = multiply_channels_indep(np.linalg.pinv(B+l*np.eye(B.shape[0],C=B.shape[1])))
    else:
...
def get_vis_one_pixel_stride(param_img,deg,knots,Ti,Tf,dxi,
                             points_spatial, offset_xi=0, orbit_fraction=0.5,
                             n_components_simulation=1, fname=None):
    # orbit_fraction effectively gives y_ang offset (orbit_fraction is a multiple of k/N for k integer, offset is zero)
    try:
    ...
    if os.path.isfile(fname):
        with open(fname, 'rb') as inf:
            vis, vtilde, A, J, Umean, U, xi_vis, eta_vis, xang_vis, yang_vis = pickle.load(inf)
            return vis, vtilde, A, J, Umean, U, xi_vis, eta_vis, xang_vis, yang_vis
    ...
    dxi = np.diff(np.unique(np.around(xis_all, thresh)))[0]
    ...
    Nvis = Nant**2
    vis = np.zeros((Nvis,Tf-Ti+1), dtype=np.complex128)
    ...
    J=J_xi_ya(rhos, eta_vis, ts, xang_vis, yang_vis)
    A = assign_and_resample_antenna_pattern(ps, np.pi-ts, points_spatial, dxi, dy, seed=10)
    Umean, U, s, Vt = get_orthogonal_model_and_mean((np.pi-ts)*360/(2*np.pi), angles=angles, pcs=pcs, 
                                                    mean_vec=mean_vec, n_components_simulation=n_components_simulation, 
                                                    kind='cubic', ablation=False)
    ...
    for ssp_i in range(Ti, Tf+1):
        ...
        for k in range(Nant):
            Ak = A[tuple(points_spatial[k])].copy()
            for l in range(Nant):
                Al = A[tuple(points_spatial[l])].copy()
                u,v = points_spatial[k]-points_spatial[l]
                cexp = np.exp(2*np.pi*1j*(u*xi_vis + v*eta_vis))
                #print("A", Ak.shape, Al.shape, "Cexp", cexp.shape, "J", J.shape )
                all_but_temp = Ak*np.conj(Al)*J*cexp
                vis[k*Nant+l, ssp_i-Ti] = all_but_temp @ temp_map
    vtilde = np.fft.fft(vis, axis=1)
    with open(fname, 'wb') as f:
        pickle.dump([vis, vtilde, A, J, Umean, U, xi_vis, eta_vis, xang_vis, yang_vis], f)

    return vis, vtilde, A, J, Umean, U, xi_vis, eta_vis, xang_vis, yang_vis

def invert_vis_one_pixel_stride(vis,deg,knots,Ti,Tf,Mh,No,C,xa_rs,ya_rs,xi_rs,eta_rs,A,J,U,dxi,points_spatial, offset_xi=0,orbit_fraction=0.5,
                                n_components_inversion=1,thresh=10,ablation_cexp=False, fname=None):
    if fname is None:
        fname = f"inv_{len(knots)}_{Ti}_{Tf}_{Mh}_{No}_{C}_{n_components_inversion}.pkl"
    if os.path.isfile(fname):
        with open(fname, 'rb') as inf:
            param_est, I, param_img, Gs, param_est_full = pickle.load(inf)
            return param_est, I, param_img, Gs, param_est_full
    orbit_fractions = np.arange(Ti,Tf+1)/No
    T = Tf-Ti+1
    ...
    for omega in range(T):
        ...
        for i in range(Nant):
            A1 = A[tuple(points_spatial[i])].copy()
            for j in range(Nant):
                A2 = A[tuple(points_spatial[j])].copy()
                u,v = np.subtract(points_spatial[j], points_spatial[i])
                cexp = np.exp(-2*np.pi*1j*(u*xi_rs + v*eta_rs))
                if ablation_cexp:
                    cexp = 1
                # TODO: is it correct (index?), exp(1j*omega*yangs_sample)???
                xy = A1*np.conj(A2)*J*cexp*np.exp(1j*omega*ya_rs)
                row = np.zeros((Mh*n_components_inversion,), dtype=np.complex128)
                for ii,ya in enumerate(all_ys):
                    mask_idxs = np.abs(np.around(ya_rs,thresh) - ya) < 10**(-1*thresh) # boolean of length equal to the number of points in resampled grid (uniform xi-ya)
                    mask_idx_numbers = np.flatnonzero(mask_idxs)
                    mask_idxs = mask_idx_numbers[xa_rs[mask_idxs].argsort()]
                    assert len(mask_idxs) == len(mask_idx_numbers) and len(set(mask_idxs).difference(set(mask_idx_numbers))) == 0
                    assert np.all(np.abs(ya-np.around(ya_rs[mask_idxs],thresh)) < 10**(-1*thresh))
                    assert ascending(xa_rs[mask_idxs])
                    xys = xy[mask_idxs]
                    xvals = xa_rs[mask_idxs]
                    for c in range(n_components_inversion):
                        std = np.sqrt(var[c])
                        Tc = std*U[:,c]
                        Tcs = Tc[mask_idxs]
                        QTM = (xys*Tcs).reshape((1,len(mask_idxs)))@Ms[ii]
                        row[Mh*c:Mh*(c+1)] += QTM.flatten()
                G[i*Nant+j,:] = row

        #Ginv = invert(G, inversion_type=inversion_type, inversion_params=inversion_params)
    ...
    with open(fname, 'wb') as f:
        pickle.dump([param_est, I, param_img, Gs, param_est_full], f)
...
for Nx in [33, 33]:
    for Ny in [33,17]:
        ...
        # major params
        for decimation_factor_x in [1]:
            for decimation_factor_y in [1]:
                for number_components in [1,2,3]:
                    # import image 
                    ...
                    for dxi in [0.02, 0.01, 0.005]:
                        ...
                        for number_knots in [1,3,5,7,9,12,15,20]:
                            for deg in [3]:
                                for regularization in [False, True, True]:
                                    for l_ in [0, 0.01, 0.1]:
                                        ...
                                        u,s,vh = np.linalg.svd(B @ np.linalg.pinv(B))
                                        plt.figure()
                                        plt.plot(1+np.arange(len(s)), s)
                                        ...
                                        plt.figure()
                                        plt.plot(1+np.arange(len(s)), np.log10(s))
                                        plt.xlabel("Singular value number")
                                        ...
                                        if len(img_horizon.shape) == 2:
                                            ...
                                            plt.colorbar()
                                            plt.savefig(f"visible_strip_dfx_{decimation_factor_x}_dfy_{decimation_factor_y}_component_1_out_of_1.png")
                                        elif len(img2.shape) ==3:
                                            for c in range(img_horizon.shape[2]):
                                                ...
                                                plt.colorbar()
                                                plt.savefig(f"visible_strip_dfx_{decimation_factor_x}_dfy_{decimation_factor_y}_component_{c}_out_of_{img2.shape[2]}.png")
                                        ...
                                        if len(param_img.shape) == 2:
                                            ...
                                            plt.figure()
                                            plt.imshow(np.abs(B@param_img-img_horizon))
                                            plt.colorbar()
                                            ...
                                            for c in range(param_img.shape[2]):
                                                ...
                                                plt.axis('off')
                                                plt.title(f"Image of spline weights on BT params, param no {c+1}")
                                                plt.colorbar()
                                                ...
                                                plt.imshow(np.abs(multiply_channels_indep(B,param_img)-img_horizon)[:,:,0])
                                                plt.colorbar()
                                                plt.title("Absolute spline interpolation error")
                                        ...
                                        for Ti, Tf in zip([0], [100]):
                                            Mh = param_img.shape[0]
                                            No = param_img.shape[1]
                                            C = 1 if len(param_img.shape) == 2 else param_img.shape[2]
                                            fname = f"rect_Ny_{Ny}_Nx_{Nx}_du_{du}_dv_{dv}_dfx_{decimation_factor_x}_dfy_{decimation_factor_y}_reg_{regularization}_lambda_{l_}_nknots_{number_knots}_deg_{deg}_numbcomp_{number_components}.pkl"
                                            vis, vtilde, A, J, Umean, U, xi_vis, eta_vis, xang_vis, yang_vis = get_vis_one_pixel_stride(param_img,deg,knots,Ti,Tf,dxi,points_spatial=points_spatial,offset_xi=offset_xi,orbit_fraction=0.5,
                                            n_components_simulation=number_components, fname="vis_"+fname)

                                            param_est, I, param_img, Gs, param_est_full = invert_vis_one_pixel_stride(vis,deg,knots,Ti,Tf,Mh,No,C,
                                       xang_vis,yang_vis,xi_vis,eta_vis,
                                       A,J,U,dxi,points_spatial=points_spatial,offset_xi=offset_xi,orbit_fraction=orbit_fraction,n_components_inversion=number_components,thresh=thresh,ablation_cexp=False,fname="inv_"+fname)

                                            if len(param_est.shape) == 2:
                                                plt.figure()
                                                plt.imshow(param_est)
                                                plt.axis('off')
                                                plt.title("Image of spline weights on BT params")
                                                plt.colorbar()
                                                plt.savefig( f"recovered_param_img_rectangle_Nx_{Nx}_Ny_{Ny}_du_{du}_dv_{dv}_reg_{regularization}_lambda_{l_}_nknots_{number_knots}_deg_{deg}_component_1_out_of_1.png")

                                                plt.figure()
                                                plt.imshow(np.abs(param_est-param_img[:,Ti:Tf+1]))
                                                plt.axis('off')
                                                plt.title("Error image of spline weights on BT params")
                                                plt.colorbar()
                                                plt.savefig(f"error_in_param_img_rectangle_Nx_{Nx}_Ny_{Ny}_du_{du}_dv_{dv}_reg_{regularization}_lambda_{l_}_nknots_{number_knots}_deg_{deg}_component_1_out_of_1.png")


                                            elif len(param_img.shape) ==3:
                                                for c in range(param_img.shape[2]):
                                                    plt.figure()
                                                    plt.imshow(param_est[:,:,c])
                                                    plt.axis('off')
                                                    plt.title("Image of spline weights on BT params")
                                                    plt.colorbar()
                                                    plt.savefig( f"recovered_param_img_rectangle_Nx_{Nx}_Ny_{Ny}_du_{du}_dv_{dv}_reg_{regularization}_lambda_{l_}_nknots_{number_knots}_deg_{deg}_component_{c}_out_of_{param_img.shape[2]}.png")

                                                    plt.figure()
                                                    plt.imshow(np.abs(param_est[:,:,c]-param_img[:,Ti:Tf+1,c]))
                                                    plt.axis('off')
                                                    plt.title(f"Error image of spline weights on BT params, param no {c+1}")
                                                    plt.colorbar()
                                                    plt.savefig(f"error_in_param_img_rectangle_Nx_{Nx}_Ny_{Ny}_du_{du}_dv_{dv}_reg_{regularization}_lambda_{l_}_nknots_{number_knots}_deg_{deg}_component_{c}_out_of_{param_img.shape[2]}.png")



                                                    plt.figure()
                                                    plt.imshow(B @ param_est[:,:,c])
                                                    plt.axis('off')
                                                    plt.title(f"Recovered BT params, param no {c+1}")
                                                    plt.colorbar()
                                                    plt.savefig(f"resampled_param_img_rectangle_Nx_{Nx}_Ny_{Ny}_du_{du}_dv_{dv}_reg_{regularization}_lambda_{l_}_nknots_{number_knots}_deg_{deg}_component_1_out_of_1.png")

                                                    plt.figure()
                                                    plt.imshow(B@np.abs(param_est-param_img[:,Ti:Tf+1]))
                                                    plt.axis('off')
                                                    plt.title(f"Error in interpolated BT params, param no {c+1}")
                                                    plt.colorbar()
                                                    plt.savefig(f"error_in_interpolated_img_rectangle_Nx_{Nx}_Ny_{Ny}_du_{du}_dv_{dv}_reg_{regularization}_lambda_{l_}_nknots_{number_knots}_deg_{deg}_component_1_out_of_1.png")

...
def invert_vis_single_snapshot(vis,deg,knots,Ti,Mh,No,C,xa_rs,ya_rs,xi_rs,eta_rs,hkl,U,points_spatial,offset_xi=0,orbit_fraction=0.5,n_components_inversion=1,thresh=10,ablation_cexp=False,fname=None,var=var):
	if os.path.isfile(fname):
		with open(fname, 'rb') as inf:
			param_est, I, G = pickle.load(inf)
			return param_est, I, G
	...
	G = np.zeros((Nvis, Nys*Mh*n_components_inversion), dtype=np.complex128)
	Ipartial = np.zeros((Nys*Mh*n_components_inversion,), dtype=np.complex128)
	for ii,ya in enumerate(all_ys):
		...
		for c in range(n_components_inversion):
			...
			QTM = (xys*Tcs)@Ms[ii]
			G[:,c*Mh*Nys+Mh*ii:Mh*c*Nys+Mh*(ii+1)] = QTM

	Ginv = np.linalg.pinv(G)
	I = np.real(Ginv @ vis)
	param_est = np.zeros((Mh, Nys, n_components_inversion))

	for iter_c in range(n_components_inversion):
		for iter_y in range(Nys):
			param_est[:,iter_y,iter_c] = I[iter_c*Mh*Nys+iter_y*Mh:iter_c*Mh*Nys+(iter_y+1)*Mh]

	with open(fname, 'wb') as f:
		pickle.dump([param_est, I, G], f)
...
for Nx in [33]:
	for Ny in [17,9,5,3]:
		try:
			...
			# major params
			for decimation_factor_x, decimation_factor_y in zip([3,2],[3,2]):
				for number_components in [3]:
					...
					for dxi in [0.02, 0.01, 0.005]:
						...
						for number_knots in [80, 100, 150]:
                            deg = 3
                            regularization=True
                            for l_ in [0.001, 0.01, 0.1, 1, 10]:
								...
								for Ti in [param_img.shape[1]//2]:
									...
									param_est, I, G = invert_vis_single_snapshot(vis,deg,knots,Ti,Mh,No,C,xang_vis,yang_vis,xi_vis,eta_vis,hkl,U,points_spatial,n_components_inversion=number_components,thresh=10,ablation_cexp=False,fname="vec_inv_"+fname)
									...
									if len(param_est.shape) == 2:
										...
										plt.close()
									...
									elif len(param_est.shape) ==3:
											C = param_est.shape[2]
											width = param_est.shape[1]
											cols = [k%img_horizon.shape[1] for k in range(Ti-width//2, Ti+width//2+1)]
											
											for c in range(C):
												interpolated_channel = B @ param_est[:,:,c]
												original_channel = img_horizon[:,cols,c] 

												plt.figure()
												plt.imshow(original_channel)
												plt.axis('off')
												plt.title(f"Original BT params, param no {c+1}")
												plt.colorbar()
												plt.savefig(f"original_param_img_"+fname[:-4]+f"_component_{c+1}_out_of_{param_img.shape[2]}.png")
												plt.close()

												plt.figure()
												plt.imshow(interpolated_channel)
												plt.axis('off')
												...
												plt.figure()
												plt.imshow(clip(np.abs(interpolated_channel-original_channel)))
												plt.axis('off')
												plt.title(f"Error in Recovered BT params, param no {c+1}")
												plt.colorbar()
												plt.savefig(f"error_in_resampled_param_img_"+fname[:-4]+f"_component_{c+1}_oo_{param_img.shape[2]}.png")
												plt.close()

											pi = multiply_channels_indep(B, param_img[:,cols,:])
											pe = img_horizon[:,cols,:]
											...
											plt.figure()
											plt.imshow(temp57p5_sim)
											plt.axis('off')
											plt.colorbar()
											plt.clim(min_57,max_57)
											plt.title(r"Simulated BT at $57.5^\circ$ (K)")
											plt.savefig("simulated_temp_57p5"+fname[:-4]+".png")
											plt.close()

											plt.figure()
											plt.imshow(temp57p5_rec)
											plt.axis('off')
											plt.colorbar()
											plt.clim(min_57,max_57)
											plt.title(r"Reconstructed BT at $57.5^\circ$ (K)")
											plt.savefig("recovered_temp_57p5"+fname[:-4]+".png")

											plt.figure()
											plt.imshow(np.abs(temp57p5_rec-temp57p5_sim))
											plt.axis('off')
											plt.colorbar()
											plt.title(r"Temperature error at $57.5^\circ$ (K)")
											plt.savefig("error_temp_57p5"+fname[:-4]+".png")


											min_27, max_27 = np.around(np.min(temp27p5_sim))-1, np.around(np.max(temp27p5_sim))+1

											plt.figure()
											plt.imshow(temp27p5_sim)
											plt.axis('off')
											plt.colorbar()
											plt.clim(min_27, max_27)
											plt.title(r"Simulated BT at $27.5^\circ$ (K)")
											plt.savefig("simulated_temp_27p5"+fname[:-4]+".png")
											plt.close()

											plt.figure()
											plt.imshow(temp27p5_rec)
											plt.axis('off')
											plt.colorbar()
											plt.clim(min_27, max_27)
											plt.title(r"Reconstructed BT at $27.5^\circ$ (K)")
											plt.savefig("recovered_temp_27p5"+fname[:-4]+".png")

											plt.figure()
											plt.imshow(np.abs(temp27p5_rec-temp27p5_sim))
											plt.axis('off')
											plt.colorbar()
											plt.title(r"Temperature error at $27.5^\circ$ (K)")
											plt.savefig("recovered_temp_27p5"+fname[:-4]+".png")

											min_2, max_2 = np.around(np.min(temp2p5_sim))-1, np.around(np.max(temp2p5_sim))+1

											plt.figure()
											plt.imshow(temp2p5_sim)
											plt.axis('off')
											plt.colorbar()
											plt.clim(min_2,max_2)
											plt.title(r"Simulated BT at $2.5^\circ$ (K)")
											plt.savefig("simulated_temp_2p5"+fname[:-4]+".png")
											plt.close()

											plt.figure()
											plt.imshow(temp2p5_rec)
											plt.axis('off')
											plt.colorbar()
											plt.clim(min_2,max_2)
											plt.title(r"Reconstructed BT at $2.5^\circ$ (K)")
											plt.savefig("recovered_temp_2p5"+fname[:-4]+".png")

											plt.figure()
											plt.imshow(np.abs(temp2p5_rec-temp2p5_sim))
											plt.axis('off')
											plt.colorbar()
											plt.title(r"Temperature error at $2.5^\circ$ (K)")
											plt.savefig("recovered_temp_2p5"+fname[:-4]+".png")
